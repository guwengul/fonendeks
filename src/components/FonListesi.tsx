          ...
          <tbody>
            {filtrelenmis.map(f => {
              ...
              <tr key={`${f.fonKodu}-${f.fonTipi}`}
                ...
                <td className={`px-4 py-2.5 sticky left-0 group/row ${favori ? 'bg-amber-50/60' : 'bg-white'}`}>
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => toggleFavori(f)}
                      title={favori ? 'Favorilerden çıkar' : 'Favorilere ekle'}
                      className={`shrink-0 transition-all focus:opacity-100 p-1 -m-1 ${favori ? 'opacity-100' : 'opacity-70 hover:opacity-100 hover:scale-110'}`}>
